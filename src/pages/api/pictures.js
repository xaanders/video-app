        ...
        if (ids.length > 0) {
            try {
                    ...
                    return new Promise((resolve) => {
                        ...
                        vimeoClient.request(uri, function (error, body, status_code, headers) {
                            ...
                            if (body.data[0]?.sizes[0]?.link) {
                                videosCollection.updateOne({ _id: key }, { $set: { image: body.data[0].sizes[0].link } }).then(item => { });
                                imgsCollection.deleteOne({ _id: key });
                            } else {
                                stillWaiting.push(key);
                            }
                            resolve();
                ...
                if (stillWaiting.length > 0) {
                    res.status(200).json({ message: `Still waiting for update ${stillWaiting.join(', ')}` });
                } else {
                    res.status(200).json({ message: 'Successfully added all the pictures' });
                }
